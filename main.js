...
function openSocket() {
    const host = adapter.config.local_server_ip;
    const port = adapter.config.local_server_port;
    ...
    server = net.createServer(function (sock) {
        ...
        sock.on('data', function (data) {
            ...
            if (jsonContent) {
                ...
                const wiffi_in_config = JSONPath.query(adapter.config, '$.devices[?(@.ip=="' + ip + '")]');
...
function createBasicStates(name, ip, room, type, callback) {
    ...
    if (room) {
        adapter.addChannelToEnum('room', room, 'root', ip_to_id(ip), function (err) {
            if (err) {
                adapter.log.error('Could not add Wiffi with ' + name + ' (' + ip + ') to enum. Error: ' + err);
            }
        });
    }
