...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'Christian Vorholt';
    var email     = grunt.option('email')  || 'chvorholt@mail.com';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new wiffi-wz name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/wiffi-wz.png')) {
            fs.renameSync(__dirname + '/admin/wiffi-wz.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/wiffi-wz.html')) {
            fs.renameSync(__dirname + '/widgets/wiffi-wz.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/wiffi-wz/js/wiffi-wz.js')) {
            fs.renameSync(__dirname + '/widgets/wiffi-wz/js/wiffi-wz.js', __dirname + '/widgets/wiffi-wz/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/wiffi-wz')) {
            fs.renameSync(__dirname + '/widgets/wiffi-wz',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Wiffi-wz/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Wiffi-wz'
                        },
                        {
                            match: /Christian Vorholt/g,
                            replacement: author
                        },
                        {
                            match: /chvorholt@mail.com/g,
                            replacement: email
                        }
                    ]
                },
